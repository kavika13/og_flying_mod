...
void FlyingMode(){
	//Flapping
	if (GetInputDown(this_mo.controller_id, flapping) && this_mo.controlled) flyingMode = 1;
	//Gliding
	else if(GetInputDown(this_mo.controller_id, gliding) && this_mo.controlled) flyingMode = 2;
	//Falling
	else flyingMode = 0;
}
...
void UpdateFlying(const Timestep &in ts){
	if (flyingActive) {
		...
		if(flyingMode == 1){
			...
			if(!jump_info.hit_wall) { 
				if (this_mo.velocity.y < 0) this_mo.velocity.y+=0.1f;
				if (flap_modifier < 0 && this_mo.velocity.y < 10.0f) this_mo.velocity.y+= 0.5f;
				if (flap_counter > 50.0f) flap_modifier = -1.0f;
				else if (flap_counter < 2.0f) flap_modifier = 0.5f;
				if (tilt_modifier < 1.8f) tilt_modifier+=0.02f;
				else if(tilt_modifier > 2.0f) tilt_modifier-=0.02f;
			}
			else if(this_mo.velocity.y < 10.0f) this_mo.velocity.y+= 0.2f;
			...
			if (flyingMode == 2){
				...
				if (air_time > 2.0f) this_mo.velocity.y-=0.3f;
				if (tilt_modifier > 3.0f) tilt_modifier-=0.02f;
				}
				if (tilt_modifier < 4.5f) tilt_modifier+=0.05f;
				
			}

			else if(tilt_modifier > 1.0f) tilt_modifier-=0.02f;
		}
		...
		if (!jump_info.hit_wall) {
			...
			if (length(tilt) > tilt_cap) tilt = normalize(tilt)*tilt_cap;
		}
		
		//Roll
		if (air_time < 0.1f) roll_modifier = 0;
		vec3 flyFace = normalize(flatten(this_mo.velocity));
		vec3 cross_flyFace = cross(oldFlyFace, flyFace);

		if (roll_modifier > 1.0f) roll_modifier = 1.0f;
		else if (roll_modifier < -1.0f) roll_modifier = -1.0f;
		else  {
		float old_roll = roll_modifier;
		roll_modifier += cross_flyFace.y;
		if (abs(roll_modifier) > abs(old_roll) - 0.001f) roll_modifier*=0.97f;
		}
	...
	else if (!jump_info.hit_wall) this_mo.SetCharAnimation("jump",20.0f,0);
	
	//Reduces movement control after wall flip
	if (after_wall_flip){
		if (wall_flip_time > 0.6f)	after_wall_flip = false;
		else wall_flip_time+=ts.step();				
	}
	
	//Ledge Jumping
	if(jump_info.ledge_delay == 0.3f && WantsToJump() && !ledge_info.on_ledge)jump_info.StartWallJump(ledge_info.ledge_dir * -1.0f);
...
void FlyingAttacks() {
	...
	//Air dash attack 
	if (air_dash > 0 && length(this_mo.velocity) > 50.0f && distance(this_mo.position ,
				 ReadCharacterID(air_attack_id).position ) <= _attack_range + range_extender ) {
			has_air_dash = true;
	...
	//Swoop attack
	else if (WantsToGrabLedge() && !WantsToJump() && air_dash < 1 && length(this_mo.velocity) > 20.0f && distance(this_mo.position ,
				 ReadCharacterID(air_attack_id).position ) <= _attack_range + range_extender + .5) {
		has_air_dash = true;
		target_id = air_attack_id;
		MovementObject @char = ReadCharacterID(target_id);
		PlaySound("Data/Sounds/ambient/amb_canyon_rock_1.wav", this_mo.position);
		...
		tethered = _TETHERED_SWOOP;
		 char.Execute("SetTetherID(" + this_mo.getID() + ");" + "SetTethered(_TETHERED_DRAGGEDBODY);");		
		}
}
...
void FlyingAnimations() {
	if (!jump_info.hit_wall) {
		...
		//Gliding
			...
			if(this_mo.velocity.y < -24.0f) this_mo.SetAnimation("Data/Custom/LePetitDocteur/Animations/diving.anm", 5.0f, 0);
			else this_mo.SetAnimation("Data/Custom/LePetitDocteur/Animations/glide.anm", 5.0f, 0);
			}
		else if (air_time > 0.5f) this_mo.SetCharAnimation("jump",5.0f,0);
		else this_mo.SetCharAnimation("jump",20.0f,0);
	}
}

void AirDash() {
	if (jump_info.hit_wall || air_time < 0.1f) has_air_dash = true;
	if(!jump_info.hit_wall){
		if (has_air_dash && this_mo.controlled && tethered == _TETHERED_FREE && WantsToFlip() && !flip_info.IsFlipping() ) {
			has_air_dash = false;
			if (air_dash < 1) air_dash = 100 ;			
		}
		else if (air_dash > 0) air_dash--;

		if (air_dash > 0) {
			if (length(this_mo.velocity) < 80.0f) this_mo.velocity*=1.02f;
			if(!flip_info.IsFlipping()){
				flip_info.StartFlip();
			}
			for(int i=0; i<3; i++) SpinSpark();
		}
...
void FlyingStuff(const Timestep &in ts){
	...
	if (tethered == _TETHERED_SWOOP) {
		...
		if (!on_ground) {
			...
			for(int i = 0; i < 25; i++) char.MoveRagdollPart(drag_body_part,drag_target,100);
		}
		else if (on_ground_time < 1.0f) UnTether();
	}
	if (tethered == _TETHERED_DRAGBODY && on_ground_time < 1.0f) UnTether();
	...
	if(ledge_info.on_ledge && air_dash > 0) air_dash = 0;
}
...
vec3 target_velocity = GetTargetVelocity();
	...
	float velocity_size = this_mo.velocity.x*this_mo.velocity.x+this_mo.velocity.z*this_mo.velocity.z;
	float updated_velocity_size = (this_mo.velocity.x+target_velocity.x)*(this_mo.velocity.x+target_velocity.x)+(this_mo.velocity.z+target_velocity.z)*(this_mo.velocity.z+target_velocity.z);
	if (after_wall_flip) 
...
void SpinSpark()
{
	vec3 com = this_mo.GetCenterOfMass();
		...
		vec3 displ = normalize(vec3(RangedRandomFloat(-1.0f,1.0f),RangedRandomFloat(-1.0f,1.0f),RangedRandomFloat(-1.0f,1.0f)));
